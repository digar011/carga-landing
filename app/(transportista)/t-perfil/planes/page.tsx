...
const transportistaPlans = getPlansForRole('transportista');

export default function TransportistaPlanesPage() {
  const { subscription, loading, error, subscribe, cancel } = useSubscription();
  const [selectedPlan, setSelectedPlan] = useState<string | null>(null);
  const [cancelling, setCancelling] = useState(false);

  const currentPlanId = subscription?.estado === 'activa' ? subscription.plan : 'basico';
    ...
    <div className="mx-auto max-w-5xl px-4 py-8">
      <div className="mb-8 text-center">
        ...
        <p className="mt-2 text-gray-500">
          Elegí el plan que mejor se adapte a tus necesidades como transportista.
        </p>
      </div>
      ...
      {subscription?.estado === 'activa' && subscription.plan !== 'basico' && (
        <div className="mt-10 text-center">
          <p className="mb-3 text-sm text-gray-500">
            Si cancelás tu plan, vas a volver al plan Básico al final del período de facturación.
          </p>
          <Button
            variant="danger"
            size="sm"
